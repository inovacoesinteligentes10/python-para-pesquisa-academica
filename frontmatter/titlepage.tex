...
% Padrão geométrico inspirado em Python
\begin{scope}[
        remember picture,
        overlay,
        shift={($(current page.center)$)}, 
    ...
    % Espiral de Fibonacci (matemática/ciência)
    \begin{scope}[opacity=0.08]
        \foreach \i in {0,1,2,...,20}{
            \pgfmathsetmacro{\angle}{\i * 137.5} % Ângulo dourado
            \pgfmathsetmacro{\radius}{0.1 * sqrt(\i)}
            \fill[pythonblue] (\angle:\radius) circle (0.05);
        }
    \end{scope}
    
    % Grade hexagonal sutil (estrutura de dados)
    \begin{scope}[opacity=0.06]
        \foreach \x in {-2,-1,0,1,2}{
            \foreach \y in {-2,-1,0,1,2}{
                \pgfmathsetmacro{\xshift}{\x + 0.5*mod(\y,2)}
                \draw[pythonyellow, very thin] 
                    (\xshift, \y*0.866) 
                    ++(0:0.3) -- ++(60:0.3) -- ++(120:0.3) 
                    -- ++(180:0.3) -- ++(240:0.3) -- ++(300:0.3) -- cycle;
            }
        }
    \end{scope}
    
    % Símbolo central minimalista
    \begin{scope}[opacity=0.12]
        % Dois círculos entrelaçados (símbolo de infinito/loops)
        \draw[pythonblue, line width=8pt] (-0.5,0) circle (0.4);
        \draw[pythonyellow, line width=8pt] (0.5,0) circle (0.4);
    \end{scope}
    
    % Pontos orbitais (algoritmos/processamento)
    \foreach \angle in {0,45,90,135,180,225,270,315}{
        \fill[pythonblue!20] (\angle:1.8) circle (0.03);
        \fill[pythonyellow!20] (\angle:2.2) circle (0.02);
    }
